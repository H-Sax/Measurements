...
# BP measurement Max(Psa)/Min(Psa), EF LV, EF RV, EF LA, EF RA, Max(Qs), Max(Qp), Max(Qav), Max(Qmv), Max(Qpv), Max(Qtv)
...
circ_time_idxs_chunked = function (p)
    ...
    for k in 1:chunks
        ...
        @time "    Results processed in" Threads.@threads for i in 1:chunksize
          ...
          out[6,i+offset] = maximum(sol[i][SA.R.q]) #  Maximum systemic flow 
          out[7,i+offset] = maximum(sol[i][PA.R.q]) #  Maximum pulmonary flow 
          out[8,i+offset] = maximum(sol[i][AV.q]) #  Maximum aortic valve flow
          out[9,i+offset] = maximum(sol[i][MV.q]) #  Maximum mitral valve flow
          out[10,i+offset] = maximum(sol[i][PV.q]) #  Maximum pulmonary valve flow
          out[11,i+offset] = maximum(sol[i][TV.q]) #  Maximum tricuspid valve flow
...
#save("3F_D.jld", "data", res)

res = load("/home/harry/Desktop/PhD/Year 3/Measurement Project/Discrete Levels/Level 3/3F/3F_D.jld")["data"]
...
@show P[29], P[28], P[22], P[3], P[4], P[2], P[19], P[14], P[15], P[18], P[31], P[11], P[8], P[13], P[26], P[33], P[34]
...
save("/home/harry/Desktop/PhD/Year 3/Measurement Project/Sloppiness Plot/Dis_levels/3f.jld","data",λ)