...
# BP measurement Max(Psa)/Min(Psa), EF LV, EF RV, EF LA, EF RA, Qs cont, Qp cont, Qav cont, Qmv cont, Qpv cont, Qtv cont, Max(Qav),     Max(Qmv), Max(Qpv), Max(Qtv)
...
circ_time_idxs_chunked = function (p)
    ...
    for k in 1:chunks
        ...
        @time "    Results processed in" Threads.@threads for i in 1:chunksize
          ...
          out[6:305,i+offset] = sol[i][SA.R.q] # Continous systemic flow 
          out[306:605,i+offset] = sol[i][PA.R.q] # Continous pulmonary flow 
          out[606:905,i+offset] = sol[i][AV.q] # Continous aortic valve flow 
          out[906:1205,i+offset] = sol[i][MV.q] # Continous mitral valve flow
          out[1206:1505,i+offset] = sol[i][PV.q] # Continous pulmonary valve flow
          out[1506:1805,i+offset] = sol[i][TV.q] # Continous tricuspid valve flow
          out[1806,i+offset] = maximum(sol[i][AV.q]) # Peak velocity aortic valve
          out[1807,i+offset] = maximum(sol[i][MV.q]) # Peak velocity mitral valve
          out[1808,i+offset] = maximum(sol[i][PV.q]) # Peak velocity pulmonary valve
          out[1809,i+offset] = maximum(sol[i][TV.q]) # Peak velocity pulmonary valve
...
#save("Level5D.jld", "data", res)

res = load("/home/harry/Desktop/PhD/Year 3/Measurement Project/Levels/Level 5/5D/Level5D.jld")["data"]
...
save("/home/harry/Desktop/PhD/Year 3/Measurement Project/Sloppiness Plot/5d.jld","data",λ)
