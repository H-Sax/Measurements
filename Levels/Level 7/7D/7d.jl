...
# BP measurement Max(Psa)/Min(Psa), EF LV, EF RV, EF LA, EF RA, Qs cont, Qp cont, Qav cont, Qmv cont, Qpv cont, Qtv cont, Max(Qav),     Max(Qmv), Max(Qpv), Max(Qtv), cont(Vlv), cont(Vrv), cont(Vla), cont(Vra), cont(Plv), cont(Psa), cont(Prv), cont(Ppa), cont(Pra), cont(Ppv)
...
circ_time_idxs_chunked = function (p)
    ...
    for k in 1:chunks
        ...
        @time "    Results processed in" Threads.@threads for i in 1:chunksize
          ...
          out[710:809,i+offset] = sol[i][3,:] # Continous right ventricular volume
          out[810:909,i+offset] = sol[i][2,:] # Continous left atrial volume
          out[910:1009,i+offset] = sol[i][4,:] # Continous right atrial volume
          out[1010:1109,i+offset] = sol[i][LV.p] # Continous left ventricular pressure
          out[1110:1209,i+offset] = sol[i][SA.C.p] # Continous systemic artery pressure
          out[1210:1309,i+offset] = sol[i][RV.p] # Continous right ventricle pressure
          out[1310:1409,i+offset] = sol[i][PA.C.p] # Continous pulmonary artery pressure
          out[1410:1509,i+offset] = sol[i][RA.p] # Continous right atrium pressure
          out[1510:1609,i+offset] = sol[i][PVe.C.p] # Continous pulmonary vein pressure
...
#save("Level7D.jld", "data", res)

res = load("/home/harry/Desktop/PhD/Year 3/Measurement Project/Levels/Level 7/7D/Level7D.jld")["data"]
...
λ = abs.(e_deomp.values)*3#/maximum(abs.(e_deomp.values)) .+1e-16
save("/home/harry/Desktop/PhD/Year 3/Measurement Project/Sloppiness Plot/7d.jld","data",λ)
