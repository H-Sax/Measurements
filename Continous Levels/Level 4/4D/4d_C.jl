...
# cont(Qs), cont(Qp), cont(Qav), cont(Qmv), cont(Qpv), cont(Qtv), cont(Vlv), cont(Vrv), cont(Vla), cont(Vra), cont(Plv), cont(Psa),    cont(Prv), cont(Ppa), cont(Pra), cont(Ppv)
...
circ_time_idxs_chunked = function (p)
    ...
    for k in 1:chunks
        ...
        @time "    Results processed in" Threads.@threads for i in 1:chunksize
          ...
          out[1201:1300,i+offset] = sol[i][RV.p] # cont right ventricular pressure
          out[1301:1400,i+offset] = sol[i][PA.C.p] # cont pulmonary artery pressure
          out[1401:1500,i+offset] = sol[i][RA.p] # cont right atrium pressure
          out[1501:1600,i+offset] = sol[i][PVe.C.p] # cont right atrium pressure
...
#save("4d_C.jld", "data", res)

res = load("/home/harry/Desktop/PhD/Year 3/Measurement Project/Continous Levels/Level 4/4D/4d_C.jld")["data"]

res.ST_Conf_Int * 1.96
res.ST
...
show(p)
#Parameters
@show P[22], P[2], P[13], P[29], P[11], P[31], P[5], P[7], P[18], P[14], P[3], P[16], P[15], P[36], P[19], P[4], P[28], P[33], P[34], P[8]
...
save("/home/harry/Desktop/PhD/Year 3/Measurement Project/Sloppiness Plot/Con_Levels/4d.jld","data",λ)